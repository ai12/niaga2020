<?php class Rab_a_detailModel extends _Model{
	public $table = "rab_a_detail";
	...
	public function GetComboParent($id_child=null){
		$row = $this->conn->GetRow("select id_rab_detail, id_rab_detail_parent, uraian from rab_a_detail where id_rab_detail = ".$this->conn->escape($id_child));
		if(!$row)
			return array();
	...
	public function GetComboChild($id_parent=null){
		...
		$rows = $this->conn->GetArray("select 
			...
			from rab_a_detail where id_rab_detail in (".implode(",", $id_rab_detailarr).")");

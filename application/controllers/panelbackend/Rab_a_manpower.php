...
class Rab_a_manpower extends _adminController{
	...
	protected function init(){
		parent::init();
		$this->viewlist = "panelbackend/rab_a_manpowerlist";
		$this->viewdetail = "panelbackend/rab_a_manpowerdetail";
	...
	public function Index($id_rab=0){
		...
		if($this->post['act']=='save_all'){
			...
			$post = $this->post;
			$this->post=array();
			foreach($post['jumlah'] as $id_manpower=>$jumlah){
				if(!$ret)
					break;

				$ret = $this->conn->goUpdate("rab_a_manpower",array("jumlah"=>$jumlah),"id_manpower = ".$this->conn->escape($id_manpower));

				if($ret){
					$this->post['day'] = $post['day'][$id_manpower];
	...
	protected function _afterDetail($id=null){
		if(empty($this->data['row']['day'])){
			...
			$rows = $this->conn->GetArray("select * from rab_a_mandays where id_manpower = ".$this->conn->escape($id));
			foreach($rows as $r){
	...
	private function _delsertMandays($id_manpower = null){
		$ret = $this->conn->Execute("delete from rab_a_mandays where id_manpower = ".$this->conn->escape($id_manpower));
		...
		if(!empty($this->post['day'])){
			foreach ($this->post['day'] as $key => $v) {
				...
				$ret = $this->conn->goInsert('rab_a_mandays', $record);
			}
		}
	...
	public function export_list($id_rab=null){
        ...
        foreach($rows as $r){
			...
			$rows1 = $this->conn->GetArray("select * from rab_a_mandays where id_manpower = ".$this->conn->escape($r['id_manpower']));

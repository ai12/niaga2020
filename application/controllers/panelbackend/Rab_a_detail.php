...
class Rab_a_detail extends _adminController{
	...
	protected function init(){
		parent::init();
		$this->viewlist = "panelbackend/rab_a_detaillist";
		$this->viewdetail = "panelbackend/rab_a_detaildetail";
		$this->template = "panelbackend/main";
		$this->layout = "panelbackend/layout_rab_analisa";
		// $this->data['width'] = "1200px";

		if ($this->mode == 'add') {
			$this->viewdetail = "panelbackend/rab_a_detailedit";
		...
		$this->load->model("Rab_a_detailModel","model");
	...
	private function _generateJasaMaterial($id_rab){
		...
		foreach($rows as $rc){
			...
			$ret = $this->conn->goInsert("rab_a_jasa_material",$rc);
		}
	...
	private function _generateManPower($id_rab){
		...
		$ret = $this->conn->Execute("insert into rab_a_manpower (id_rab, id_team_proyek, id_jabatan_proyek, jumlah, id_sumber_pegawai, id_manpower_old) select $id_rab, id_team_proyek, id_jabatan_proyek, jumlah, id_sumber_pegawai, id_manpower
				from rab_manpower where id_rab = ".$this->conn->escape($id_rab));

		if($ret){
			$ret = $this->conn->Execute("insert into rab_a_mandays (id_manpower, jumlah, day) select b.id_manpower, a.jumlah, a.day from rab_mandays a
			join rab_a_manpower b on a.id_manpower = b.id_manpower_old where id_rab = ".$this->conn->escape($id_rab));
		}
	...
	private function _generateRabDetail($id_rab){
		...
		if($id_rab){
			...
			foreach($rows as $rc){
				...
				$ret = $this->conn->goInsert("rab_a_detail",$rc);
			}
		}

		if($id_rab && $ret){
			$ret=$this->conn->execute("update rab_a_detail a 
			set id_rab_detail_parent = (select id_rab_detail from rab_a_detail b 
		...
		if($ret)
			$ret = $this->_reCalculation($id_rab);
	...
	private function _generateRabAttribute($id_rab){
		...
		$ret = $this->conn->Execute("insert into rab_a_detail_jabatan_proyek (id_rab_detail, id_jabatan_proyek) select b.id_rab_detail, a.id_jabatan_proyek 
			from rab_detail_jabatan_proyek a
			join rab_a_detail b on a.id_rab_detail = b.id_rab_detail_old 
			where b.id_rab = ".$this->conn->escape($id_rab));

		if($ret){
			$ret = $this->conn->Execute("insert into rab_a_detail_sumber_pegawai (id_rab_detail, id_sumber_pegawai) select b.id_rab_detail, a.id_sumber_pegawai 
	...
	public function go_print_detail($id_rab=null, $id_rab_detail=null){
		...
		$this->_afterDetail($id_rab_detail);
		$this->View("panelbackend/rab_a_detaildetailprint");
	}
	...
	public function Detail($id_rab=null, $id=null){
		...
		if($this->post['act']=='save_edit'){
			$this->conn->goUpdate("rab_a_detail",$record, "id_rab_detail = ".$this->conn->escape($this->post['key']));
		...
		if($this->post['act']=='save_add'){
			...
			$this->conn->goInsert("rab_a_detail",$record);
			$id = $this->conn->GetOne("select max(id_rab_detail) from rab_a_detail where id_rab_detail_parent = ".$this->conn->escape($id));
		...
		if($this->post['act']=='delete'){
			$this->conn->Execute("delete from rab_a_detail where id_rab_detail = ".$this->conn->escape($this->post['key']));
			redirect(current_url());
		}
	...
	protected function _afterDetail($id=null){
		...
		if($this->viewdetail<>"panelbackend/rab_a_detailedit"){
			if(!$this->data['row']['id_rab_detail']){
				$max_level = 3;
				$rows = $this->conn->GetArray("select * from rab_a_detail where id_rab = ".$this->conn->escape($this->data['id_rab'])." and id_item is null order by nvl(urutan, nvl(id_rab_detail_parent, id_rab_detail)), kode_biaya");
			}else{
				$max_level = 2;
				...
				$rows = $this->conn->GetArray("select * from rab_a_detail where id_rab = ".$this->conn->escape($this->data['id_rab'])." and id_rab_detail in (".implode(",", $id_rab_detailarr).") and id_rab_detail <> ".$this->conn->escape($this->data['row']['id_rab_detail'])." order by kode_biaya, id_rab_detail");
			}
		...
		if($this->data['row']['sumber_satuan']==2 or $this->data['row']['sumber_satuan']==3){
			if($this->post['act']!='set_value' && empty($this->post['id_jabatan_proyek'])){
				$this->data['row']['id_jabatan_proyek'] = $this->conn->GetList("select id_jabatan_proyek as key, id_jabatan_proyek as val 
					from rab_a_detail_jabatan_proyek 
			...
			if($this->post['act']!='set_value' && empty($this->post['id_sumber_pegawai'])){
				$this->data['row']['id_sumber_pegawai'] = $this->conn->GetList("select id_sumber_pegawai as key, id_sumber_pegawai as val 
					from rab_a_detail_sumber_pegawai 
					where id_rab_detail = ".$this->conn->escape($id));
	...
	public function go_print($id_rab=null){
		...
		$this->_beforeDetail($id_rab);
		$this->_afterDetail();
		$this->View("panelbackend/rab_a_detailprint");
	}
	...
	protected function _beforeDetail($id_rab=null, $id=null){
		...
		$this->data['id_rab_detail_evaluasi'] = $this->conn->GetOne("select id_rab_detail from rab_a_detail 
			where id_rab = ".$this->conn->escape($this->id_rab)." 
			and id_rab_detail_parent is null and sumber_nilai = 1");
	...
	private function _delsertJabatanProyek($id_rab_detail = null){
		$ret = $this->conn->Execute("delete from rab_a_detail_jabatan_proyek where id_rab_detail = ".$this->conn->escape($id_rab_detail));
		...
		if(is_array($this->post['id_jabatan_proyek']) && count($this->post['id_jabatan_proyek'])){
			foreach($this->post['id_jabatan_proyek'] as $k=>$v){
				...
				$this->conn->goInsert("rab_a_detail_jabatan_proyek", $record);
			}
		}
		return $ret;
	}
	...
	private function _delsertSumberPegawai($id_rab_detail = null){
		$ret = $this->conn->Execute("delete from rab_a_detail_sumber_pegawai where id_rab_detail = ".$this->conn->escape($id_rab_detail));
		...
		if(is_array($this->post['id_sumber_pegawai']) && count($this->post['id_sumber_pegawai'])){
			foreach($this->post['id_sumber_pegawai'] as $k=>$v){
				...
				$this->conn->goInsert("rab_a_detail_sumber_pegawai", $record);
			}
		}
		return $ret;
	}
...
/*
	...
	private function hpp($id_rab=null){
		...
		$this->data['id_rab'] = $this->id_rab = $this->data['rowheader3']['id_rab'];

		$id_rab_max = $this->conn->GetOne("select max(id_rab) from rab_a_detail where id_rab<>".$this->conn->escape($this->id_rab));

		#biaya produksi
		$rows = $this->conn->GetArray("select * from rab_a_detail where id_rab = ".$this->conn->escape($this->id_rab));
		...
		foreach($rows as $r){
			if($r['sumber_nilai']==2){
				...
				if(!$this->data['id_rab_detail_parent'])
					$this->data['id_rab_detail_parent'] = $r['id_rab_detail_parent'];
			}

			if(!$this->data['id_rab_detail_parent'] && $r['uraian']=='RAB KOMERSIAL')
				$this->data['id_rab_detail_parent'] = $r['id_rab_detail'];

			if(!$r['id_rab_detail_parent'])
				$this->data['header'][$r['uraian']] = $r;
		}

		if(!$this->data['header']['RAB EVALUASI']){
			$record = array();
			$record['uraian'] = 'RAB EVALUASI';
			$record['sumber_nilai'] = 1;
			$record['id_rab'] = $this->data['id_rab'];
			$this->conn->goInsert("rab_a_detail",$record);
			$this->data['id_rab_detail_parent'] = $this->conn->GetOne("select max(id_rab_detail) from rab_a_detail where id_rab = ".$this->conn->escape($this->id_rab)." and sumber_nilai = 1");
		}
		...
		if(!$this->data['header']['OTHER COST']){
			...
			$rows = $this->conn->GetArray("select * from rab_a_detail where id_rab = ".$this->conn->escape($id_rab_max));
			...
			foreach($rows as $r){
				if(!$r['id_rab_detail_parent'])
					...
					$child[$r['id_rab_detail_parent']][] = $r;
			}
	...
	private function insertProduksiOther($arr, $rows, $id_rab_detail_parent){
		foreach($rows as $record){
			$record['id_rab_detail_parent'] = $id_rab_detail_parent;
			$record['id_rab'] = $this->data['id_rab'];
			$id_rab_detail_parent_old1 = $record['id_rab_detail'];
			unset($record['id_rab_detail']);
			$this->conn->goInsert("rab_a_detail",$record);

			$id_rab_detail_parent1 = $this->conn->GetOne("select max(id_rab_detail) from rab_a_detail where id_rab = ".$this->conn->escape($this->id_rab)." and sumber_nilai = ".$this->conn->escape($record['sumber_nilai']));

			if($arr[$id_rab_detail_parent_old1]){
				$this->insertProduksiOther($arr, $arr[$id_rab_detail_parent_old1], $id_rab_detail_parent1);
			}
		}
	}
}
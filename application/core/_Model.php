...
class _Model extends CI_Model{
	...
	function SqlCombo($where=''){
		return "select {$this->pk} as key, {$this->label} as val from {$this->table} $where order by key";
	}

	function SqlComboDK($where = ''){
		return "select {$this->pk} as key, {$this->kode} as kode, {$this->label} as val from {$this->table} $where order by key";
	}
	...
	function GetComboDK($where=''){
		$sql = $this->SqlComboDK($where);
		$rows = $this->conn->GetArray($sql);
		$data = array(''=>'-pilih-');
		foreach ($rows as $r) {

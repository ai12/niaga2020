...
class CI_DB extends TEMP_DB {
	...
	public function InsertSQL($table=false, $arr_data=array()){
		...
		foreach ($cols as $key => $value) {
			...
			switch ($value['type']){
				...
				break;
				case "TIMESTAMP(6)":
					if (strtolower($arr_data[$key]) == '{{sysdate}}' || strtolower($arr_data[$key]) == '{{current_timestamp}}') {
						$values[] = $this->qstr($arr_data[$key]);
					}
					else {
						$values[] = "TO_TIMESTAMP(".$this->qstr($arr_data[$key]).",'$timestamp_format')";
					}
				break;
				default:
					$values[] = $this->qstr($arr_data[$key]);
				...
				switch ($value['type']){
					...
					case "TIMESTAMP(6)":
						if (strtolower($arr_data[$key]) == '{{sysdate}}' || strtolower($arr_data[$key]) == '{{current_timestamp}}') {
							...
							$updatearr[] = $key." = ". "TO_TIMESTAMP(".$this->qstr($arr_data[$key]).",'$timestamp_format')";
						}
					break;
					default:
						$updatearr[] = $key." = ". $this->qstr($arr_data[$key]);
	...
	public function goInsert($table=false, $arr_data=array(), $is_log=true){
		...
		if($is_log){
			$this->log("menambah data di tabel ".$table." ".json_encode($arr_data));
		}
	...
	protected function log($act=""){
		...
		$record = array();
		$record['page'] = $_SERVER['REQUEST_URI'];
		$record['activity'] = $act;
		...
		$this->goInsert("public_sys_log",$record, false);
	}

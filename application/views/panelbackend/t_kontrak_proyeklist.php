<table class="table table-striped table-hover dataTable">
				...
				foreach($list['rows'] as $rows){
					...
					foreach($header as $rows1){
						...
						if($rows1['name']=='nama'){
							if($add_param)
								echo "<td><a href='".($url=base_url("panelbackend/t_proyek_kontrak/detail/$add_param/$rows[$pk]"))."'>$val</a></td>"; 
							else  
								echo "<td><a href='".($url=base_url("panelbackend/t_proyek_kontrak/detail/$rows[$pk]"))."'>$val</a></td>";   
						}elseif($rows1['name']=='kontrak_nilai'){

<ol class="breadcrumb" style="margin-bottom: 0px">
  <li><a href="<?=site_url("panelbackend/rab_e_detail/index/$id_rab")?>">RAB</a></li>
  <?php
  foreach($breadcrumb as $k=>$v){
  	echo '<li><a href="'.site_url("panelbackend/rab_e_detail/detail/$id_rab/$k").'">'.$v.'</a></li>';
  }
  ?>
  <li><?=$row['uraian']?></li>
</ol>
	...
	<tbody>
		...
		<tr>
			...
			<?php if($r[$pk]==$this->post['key'] && $this->post['act']=='edit'){ ?>
				...
				<?php if($is_satuan){ 
					if(!$r['day'])$r['day']=1;
					if($r['sumber_satuan']==1){
					?>
					<td style="text-align: center;">
						<input style="width:100%; text-align: right;" type="number" name="vol" value="<?=((float)$r['vol']*(float)$r['day'])?(float)$r['vol']*(float)$r['day']:''?>"/>
					</td>
					<td style="text-align: center;">
			...
			<?php }else{ ?>
				...
				<?php if($is_kode_biaya){ ?>
					<td><?=$r['kode_biaya']?></td>
				<?php } ?>
				<td><a href='<?=site_url("panelbackend/rab_e_detail/detail/$id_rab/$r[id_rab_detail]")?>'><?=$r['uraian']?></a></td>
				<?php if($is_satuan){ 
					if(!$r['day'])$r['day']=1;
					?>
					<td style="text-align: center;"><?=((float)$r['vol']*(float)$r['day'])?(float)$r['vol']*(float)$r['day']:''?></td>

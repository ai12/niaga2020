<table class="table table-bordered  table-hover">
	...
	<?php
	$id_parent = $header['RAB EVALUASI']['id_biaya_produksi'];
	$no=1;
	$total = 0;
	if($isi[$id_parent]['2'])
...
<?php $this->conn->goUpdate("proyek_pekerjaan",array("nilai_hpp"=>$this->conn->escape($total_rab_evaluasi)),"id_proyek_pekerjaan = ".$this->conn->escape($id_proyek_pekerjaan)); ?>
<table class="table table-bordered  table-hover">
	<tr>
		<th width="40px">B</th>
	...
	<?php
	$id_parent = $header['OTHER COST']['id_biaya_produksi'];
	$no=1;
	$total = 0;
	if($isi[$id_parent]['1'])
	foreach($isi[$id_parent]['1'] as $r){ 
		$id_parent = $r['id_biaya_produksi'];
	?>
		...
		foreach($isi[$id_parent]['5'] as $r){ 
			$menuarr = array();
			$menuarr[] = " <li><a href=\"javascript:void(0)\" class=\"waves-effect\" onclick=\"goSubmitValue('edit', $r[id_biaya_produksi])\" ><span class=\"glyphicon glyphicon-edit\"></span> Edit</a> </li> ";
			$menuarr[] = " <li><a href=\"javascript:void(0)\" class=\"waves-effect\" onclick=\"goSubmitValue('delete', $r[id_biaya_produksi])\" ><span class=\"glyphicon glyphicon-remove\"></span> Delete</a> </li> ";
		?>

			<?php if($r['id_biaya_produksi']==$this->post['key'] && $this->post['act']=='edit'){ ?>
				<tr>
					<td>&nbsp;&nbsp;&nbsp;<?=$noa++?></td>
					...
					<td align="right" style="padding-top: 5px !important;"><a href="javascript::void(0)" onclick="goSubmitValue('save_edit',<?=$r['id_biaya_produksi']?>)"><span class="glyphicon glyphicon-floppy-save"></span></a></td>
...
<table class="table table-bordered  table-hover">
	...
	<?php
	$id_parent = $header['SALARY PERSONIL PJBS']['id_biaya_produksi'];
	$no=1;
	$total = 0;
	if($isi[$id_parent]['3'])
...
<table class="table table-bordered  table-hover">
	...
	<tr>
		...
		<?php
		$this->conn->goUpdate("rab_evaluasi",array("hpp"=>$t_hpp),"id_rab_evaluasi = ".$this->conn->escape($id_rab_evaluasi));
		?>
	</tr>
</table>
<style type="text/css">

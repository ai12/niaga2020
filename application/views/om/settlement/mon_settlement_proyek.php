...
<div class="container">
	<form method="post" enctype="multipart/form-data" id="main_form" class="form-horizontal">
		...
		<section class="content-header" style="max-width:1800px; margin-right:auto; margin-left:auto;">
			<h1>
				Monitoring Settlement Proyek</h1>
		</section>
					...
					<div style='overflow-x:scroll;overflow-y:hidden;width:100%;'>
						<table id="table_settlement" border="0" cellpadding="0" cellspacing="0" class="table table-bordered table-hover dataTable">
							...
							<tbody>
								...
								unset($kontrakarr['']);
								// unset($units['***']);
								foreach ($kontrakarr as $key => $value) {
									foreach ($arrPeriode as $k => $dt) {
										$settlement[$k] = $this->Global_model->nilai_settlement($key, $dt, 1);
								...
								?>
									<tr>
										...
										<?php foreach ($arrPeriode as $k => $dt) {
											...
											echo '<td nowrap style="background-color:' . $slaColor . ';text-color:black;text-align:right"><a style="color:white" href="#" onClick="openModal(\'' . base_url('panelbackend/mon_proyeksettlement/nilai/' . $key . '/' . $dt) . '\')">' . $slaVal . '</a></td>';
											// echo '<td nowrap style="text-align:right">'.$settlement[$k]['status'].'</td>';
										} ?>
										<td></td>
										<td rowspan="5" style="vertical-align:top;width:200px" nowrap><?php echo $this->Global_model->uraian_settlement($key, 1, 1, $arrPeriode); ?> <a href="#" onClick="openModal2('<?php echo base_url('panelbackend/mon_proyeksettlement/history/' . $key . '/1') ?>')">read more</a></td>
										<td rowspan="5" style="vertical-align:top;width:200px" nowrap><?php echo $this->Global_model->uraian_settlement($key, 2, 1, $arrPeriode); ?> <a href="#" onClick="openModal2('<?php echo base_url('panelbackend/mon_proyeksettlement/history/' . $key . '/2') ?>')">read more</a></td>
								...
								<tr>
										...
										?>
										<td></td>
										<td rowspan="5" style="vertical-align:top;width:200px" nowrap><?php //echo $this->Global_model->uraian_settlement($key, 1, 1, $arrPeriode); ?></td>
										<td rowspan="5" style="vertical-align:top;width:200px" nowrap><?php //echo $this->Global_model->uraian_settlement($key, 2, 1, $arrPeriode); ?></td>
...
<script>
	function print() {
		...
		$("#table_settlement").table2excel({
			// exclude CSS class
			name: "Worksheet Name",
			filename: "Settlement Proyek", //do not include extension
	...
	function update() {
		...
		window.location = "<?php echo base_url('panelbackend/mon_proyeksettlement/index') ?>" + '/' + awal + '/' + akhir;
	}

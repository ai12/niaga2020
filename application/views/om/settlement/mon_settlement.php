				...
				<div class="box-body">
					...
					<table>
						<tr>
                            ...
                            <button type="button" class="btn btn-success" onClick="update2()">Laporan Radir</button>
					...
					<div style='overflow-x:scroll;overflow-y:hidden;width:100%;'>
						<table id="table_settlement" border="0" cellpadding="0" cellspacing="0" class="table table-bordered table-hover dataTable">
							...
							<tbody>
								...
								?>
									<tr>
										<td rowspan="7"><?php echo $no;
														$no++; ?></td>
										<td rowspan="7"><?php echo $value; ?></td>
										<td>STATUS</td>
										...
										<td></td>
										<td rowspan="7" style="vertical-align:top;width:200px" nowrap><?php echo $this->Global_model->uraian_settlement($key, 1, 2 ,$arrPeriode); ?> <a href="#" onClick="openModal2('<?php echo base_url('panelbackend/mon_settlement/history/' . $key . '/1') ?>')">read more</a></td>
										<td rowspan="7" style="vertical-align:top;width:200px" nowrap><?php echo $this->Global_model->uraian_settlement($key, 2, 2 ,$arrPeriode); ?> <a href="#" onClick="openModal2('<?php echo base_url('panelbackend/mon_settlement/history/' . $key . '/2') ?>')">read more</a></td>

									</tr>
									<tr>
										<td>NILAI KONTRAK<br>(inc PPN)</td>
										<?php
										$total = 0;
										foreach ($arrPeriode as $k => $dt) {
									...
									<tr>
										<td> NILAI TAGIHAN <br>(inc PPN)</td>
										<?php
										$total = 0;
										foreach ($arrPeriode as $k => $dt) {
									...
									<tr>
										<td>NILAI TERBAYAR<br>(exc pajak, PPN, denda)</td>
											<?php
										$total = 0;
										foreach ($arrPeriode as $k => $dt) {
									...
									<tr>
										...
										$total = 0;
										foreach ($arrPeriode as $k => $dt) {
											$total += $settlement[$k]['nilai_hpp'];
											echo '<td nowrap style="text-align:right">Rp.' . number_format($settlement[$k]['nilai_hpp'], 0, '.', ',') . '</td>';
										}
										echo '<td nowrap style="text-align:right">Rp.' . number_format($total, 0, '.', ',') . '</td>';
										?>
									</tr>
								...
								<tr>
										<td rowspan="7" colspan="2">Total Periode</td>
										<td>&nbsp;</td>
										<?php foreach ($arrPeriode as $k => $dt) {
											$slaVal = $settlement[$k]['status'];
											$slaColor = $arrSlaColor[$slaVal];
										...
										<td></td>
										<td rowspan="6" style="vertical-align:top;width:200px" nowrap><?php //echo $this->Global_model->uraian_settlement($key, 1); ?></td>
										<td rowspan="6" style="vertical-align:top;width:200px" nowrap><?php //echo $this->Global_model->uraian_settlement($key, 2); ?></td>
									...
									<tr>
										...
										foreach ($arrPeriode as $k => $dt) {
											$total += $total_biaya[$k];
											echo '<td nowrap style="text-align:right">Rp.' . number_format($total_biaya[$k], 0, '.', ',') . '</td>';
										}
										echo '<td nowrap style="text-align:right">Rp.' . number_format($total, 0, '.', ',') . '</td>';
										?>
									</tr>
									<tr>
										...
										foreach ($arrPeriode as $k => $dt) {
											$total += $total_hpp[$k];
											echo '<td nowrap style="text-align:right">Rp.' . number_format($total_hpp[$k], 0, '.', ',') . '</td>';
										}
										echo '<td nowrap style="text-align:right">Rp.' . number_format($total, 0, '.', ',') . '</td>';
										?>
									</tr>
...
<script>
	function print() {
        ...
        var url = "<?php echo base_url('panelbackend/mon_settlement/detail') ?>" + '/' + awal + '/' + akhir;

		// window.location = "<?php echo base_url('panelbackend/mon_settlement/detail') ?>" + '/' + awal + '/' + akhir;
	...
	function print2() {
		...
		$("#table_settlement").table2excel({
			// exclude CSS class
			name: "Worksheet Name",
			filename: "Settlement OM", //do not include extension
